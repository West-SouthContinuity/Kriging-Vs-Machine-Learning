...
from numpy.linalg import LinAlgError #an exception raised when a linear algebra problem is encountered
...
def cova2(x1, y1, x2, y2, nst, c0, pmx, cc, aa, it, ang, anis, rotmat, maxcov):
    ...
    :param pmx: maximum permitted value
    :param cc: multiplicative factor of each nested structure
    :param aa: parameter 'a' of each nested structure
    :param it: type code of each nested structure
    :param ang: azimuth of each nested structure
    :param anis: anisotropy ratio of each nested structure
    :param rotmat: rotation matrices
    :param maxcov: maximum covariance
    ...
    for js in range(0, nst):
        ...
        dy1 = 0.0  # Initialize dy1
        if anis[js] > EPSLON:  # Check to avoid division by zero
...
def vmodel3(
    ...
    """GSLIB's VMODEL program converted from the original Fortran to Python.

    :param nlag: number of variogram lags 
    :param xlag: size of the lags
    :param azm: direction by 2D azimuth, 000 is y positive, 090 is x positive 
    :param vario: dictionary with the variogram parameters
    :return: index, h, gam, cov, ro
    """
    # Parameters
    MAXNST = 3  # Updated to 3 for the third structure
    DEG2RAD = 3.14159265 / 180.0
    MAXROT = MAXNST + 1
    EPSLON = 1.0e-20
    VERSION = 1.01
    ...
    for i in range(nst):
        cc[i] = vario[f"cc{i+1}"]
        it[i] = vario[f"it{i+1}"]
        ang[i] = vario[f"azi{i+1}"]
        aa[i] = vario[f"hmaj{i+1}"]
        anis[i] = vario[f"hmin{i+1}"] / vario[f"hmaj{i+1}"]
...
def kb2d_3(
 ...
 # Load the variogram model parameters
    nst = vario['nst']
    cc = np.zeros(nst)
    aa = np.zeros(nst)
    it = np.zeros(nst)
    ...
    if nst == 3:
        ...
        ang[2] = vario['azi3']
        aa[2] = vario['hmaj3']
        anis[2] = vario['hmin3'] / vario['hmaj3']
    ...
    kmap = np.zeros((ny,nx))
    vmap = np.zeros((ny,nx))
    weights_map = np.zeros((ny, nx, MAXSAM))  
...
# Solve the Kriging System:
            try:
                s = ksol_numpy(neq, a, r)
                ising = 0  # Assuming ksol_numpy function sets ising or similar flag in case of issues
            except LinAlgError:
                ising = 1
                
            if ising != 0:
                print('WARNING KB2D: singular matrix or linear algebra error')
                print(f'              for block {ix},{iy}')
                est = UNEST  # Use UNEST to indicate an unsolvable/undefined estimate
                estv = UNEST  # Similarly for variance
                
            else:
# Ensure that 's' has been calculated before attempting to use it
                if 's' in locals() and len(s) > na:
                    est = 0.0
                    estv = cbb
                    sumw = 0.0
                    ...
                    if ktype == 1: 
                        estv -= (s[na]) * unbias
                    for i in range(na):  # Assuming na is the number of available samples
                        sumw += s[i]
                        est += s[i] * vra[i]
                        estv -= s[i] * rr[i]
                    if ktype == 0: 
                    ...
                    # Handle cases where 's' cannot be used for calculation
                    print(f'Error processing block {ix},{iy}: Solution array "s" is not valid.')
                    est = UNEST
                    estv = UNEST

          
            # Update maps and weights after handling
            kmap[ny-iy-1, ix] = est
            vmap[ny-iy-1, ix] = estv
            if 's' in locals() and len(s) > na:
                weights_map[iy, ix, :neq] = s[:neq]
            
            # Handling nk, ak, vk calculations or adjustments here if needed
            if est > UNEST:
                nk += 1
                ak += est
                vk += est * est
...
# END OF MAIN LOOP OVER ALL THE BLOCKS:
    ...
    return kmap, vmap, weights_map,cbb,s,vk

def grid_search_kriging(df, xcol, ycol, vcol, tmin, tmax, nx, xmn, xsiz, ny, ymn, ysiz, nxdis, nydis, ktype, skmean, vario):
    # Define ranges for the search radius, ndmin, and ndmax
    search_radii = np.linspace(2000, 5000, num= 500)
    ndmin_values = range(1, 12 + 1)
    ndmax_values = range(5, 40 + 1)

    # Placeholder for the best parameters and their performance
    best_params = {}
    best_performance = float('inf')

    # Loop over all combinations of search radius, ndmin, and ndmax
    for radius in search_radii:
        for ndmin in ndmin_values:
            for ndmax in ndmax_values:
                # Perform kriging with the current set of parameters
                kriged_results = kb2d_3(df, xcol, ycol, vcol, tmin, tmax, nx, xmn, xsiz, ny, ymn, ysiz, nxdis, nydis, ndmin, ndmax, radius, ktype, skmean, vario)

                # Evaluate the performance of the kriging results
                # Here, you need a function that calculates the performance metric, e.g., MSE
                performance_metric = calculate_performance_metric(kriged_results, actual_values)
                ...
                if performance_metric < best_performance:
                    ...
                    best_params = {'radius': radius, 'ndmin': ndmin, 'ndmax': ndmax}

    return best_params, best_performance

# Example usage:
# best_params, best_perf = grid_search_kriging(df, xcol, ycol, vcol, tmin, tmax, ...)
# print(f"Best Parameters: {best_params}, Best Performance: {best_perf}")
...
# Constants
    ...
    if nst >= 2:
        cc[1] = vario['cc2']; it[1] = vario['it2']; ang[1] = vario['azi2']
        aa[1] = vario['hmaj2']; anis[1] = vario['hmin2']/vario['hmaj2']
    if nst == 3:  # Accommodating the third structure
        cc[2] = vario['cc3']; it[2] = vario['it3']; ang[2] = vario['azi3']
        aa[2] = vario['hmaj3']; anis[2] = vario['hmin3']/vario['hmaj3']
...
# Solve the Kriging System:
        try:
            # Attempt to solve the kriging system. Assuming ksol_numpy is your function to solve the system.
            s = ksol_numpy(neq, a, r)

            # Compute the estimate and the kriging variance if the system was solved successfully:
            est = 0.0
            estv = cbb  # Initial kriging variance set to cbb
            sumw = 0.0  # Sum of weights for unbiasedness

            # Adjustments for different kriging types
            if ktype == 1:
                estv -= s[na] * unbias  # Adjusting kriging variance for ordinary kriging

            # Accumulating weights and contributions for the estimate and its variance
            for i in range(na):  # Iterate through the neighborhood samples
                sumw += s[i]
                est += s[i] * vra[i]  # Weighted sum for the estimate
                estv -= s[i] * rr[i]  # Adjustment for the kriging variance

            # For simple kriging, adjust the estimate with the mean if required
            if ktype == 0:
                est += (1.0 - sumw) * skmean  # Adjusting estimate for simple kriging

        except LinAlgError:
            print(f'WARNING KB2D: Singular matrix at location {idata} ({xloc}, {yloc})')
            est = UNEST
            estv = UNEST

        # Update lists outside of the except block
        klist[idata] = est
        vlist[idata] = estv
